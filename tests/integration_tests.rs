...
fn test_keyword_similarity_accuracy() {
    ...
    let docs = vec![
        ("design_full", vec!["fpga", "verilog", "synthesis", "timing"]),
        ("design_partial", vec!["fpga", "verilog", "hardware", "power"]),
        ("design_alt", vec!["hardware", "layout", "verification", "mask"]),
    ...
    for (doc_id, keywords) in &docs {
      ...
      // Query with hardware-related keywords ["fpga", "verilog", "synthesis", "timing"]
    ...
    assert!(design_full_sim >= design_partial_sim, "Full match should beat partial match");
    assert!(design_full_sim >= single_sim, "Full match should beat single match");
    assert!(design_full_sim >= design_alt_sim, "Full match should beat no match");
    
    // Partial match should beat no direct matches
    assert!(design_partial_sim >= design_alt_sim, "Partial match should beat no match");
    
    // Note: Due to bloom filter hash collisions, single_match vs design_alt ordering may vary
}
...
#[test]
fn test_bloom_filter_properties() {    // Test bloom filter false positive rate with hardware design keywords
    ...
    println!("Estimated false positive rate for hardware keywords: {:.4}", false_positive_rate);
