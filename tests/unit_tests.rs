...
mod symmetric_encryption_tests {
    ...
    fn test_symmetric_key_generation() {
        ...
        // We can't directly compare them, so we test encryption produces different results
...
mod os2_client_tests {
    ...
    fn test_document_outsourcing_basic() {
        ...
        let encrypted_doc = client.outsource_document(doc_id, content, keywords).unwrap();
    ...
    fn test_similarity_calculation_identical() {
        ...
        let doc = client.outsource_document("doc", "content", keywords.clone()).unwrap();
    ...
    fn test_similarity_calculation_no_overlap() {
        ...
        let doc = client.outsource_document("doc", "content", doc_keywords).unwrap();
        server.store_document(doc);
...
mod cloud_server_tests {
    ...
    fn test_server_key_setup() {
        ...
        let doc = client.outsource_document("test", "content", vec!["keyword"]).unwrap();
        server.store_document(doc);
    ...
    fn test_server_document_storage() {
        ...
        let docs = vec![
            client.outsource_document("doc1", "content1", vec!["key1"]).unwrap(),
            client.outsource_document("doc2", "content2", vec!["key2"]).unwrap(),
            client.outsource_document("doc3", "content3", vec!["key3"]).unwrap(),
        ];
    ...
    fn test_server_query_evaluation() {
        ...
        for (id, keywords) in &docs {
            let doc = client.outsource_document(id, "content", keywords.clone()).unwrap();
            server.store_document(doc);
        }
...
mod integration_tests {
    ...
    fn test_full_workflow_multiple_documents() {
        ...
        let documents = vec![
            ("doc1", "AI and machine learning research", vec!["ai", "machine", "learning", "research"]),
            ("doc2", "Machine learning applications", vec!["machine", "learning", "applications"]),
            ("doc3", "Deep learning neural networks", vec!["deep", "learning", "neural", "networks"]),
            ("doc4", "Web development tutorial", vec!["web", "development", "tutorial"]),
        ];
        ...
        let mut similarities: Vec<(String, f64)> = results.into_iter()
    ...
    fn test_edge_case_very_long_keywords() {
        ...
        let doc = client.outsource_document("doc", "content", keywords).unwrap();
    ...
    fn test_unicode_keyword_handling() {
        ...
        let doc = client.outsource_document("unicode_doc", "Unicode content", unicode_keywords).unwrap();
        assert!(doc.encrypted_index.tau > 0);
