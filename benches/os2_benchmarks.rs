    ...
    group.bench_function("enterprise_scenario", |b| {
        b.iter_batched(
            ...
            |(client, mut server, documents)| {
                ...
                let query_bf = client.generate_query_bloom_filter(vec!["project", "development", "software"]);
                let results = server.evaluate_query(&query_bf);
...
fn crypto_operations_for_hardware_benchmark(c: &mut Criterion) {
    ...
    for key_size in [1024, 2048, 4096].iter() {
        ...
        group.bench_with_input(
            ...
            |b, (pk, message)| {
                b.iter(|| {
                    encrypt_paillier(black_box(message), black_box(pk))
                });
            },
        );
        ...
        group.bench_with_input(
            ...
            |b, (sk, pk, ciphertext)| {
                b.iter(|| {
                    decrypt_paillier(black_box(ciphertext), black_box(sk), black_box(pk))
                });
            },
        );
        ...
        group.bench_with_input(
            ...
            |b, (pk, c1, c2)| {
                b.iter(|| {
                    add_homomorphic(black_box(c1), black_box(c2), black_box(pk))
                });
            },
        );
    }
    ...
    for batch_size in batch_sizes.iter() {
        ...
        group.throughput(Throughput::Elements(*batch_size as u64));        group.bench_with_input(
        ...
        let ciphertexts: Vec<BigUint> = messages.iter()
            .map(|m| encrypt_paillier(m, &pk))
            .collect();
            ...
            |b, (sk, pk, ciphertexts)| {
                b.iter(|| {
                    for ciphertext in ciphertexts.iter() {
                        black_box(decrypt_paillier(black_box(ciphertext), black_box(sk), black_box(pk)));
                    }
                });
            },
        );
...
fn bloom_filter_hardware_benchmark(c: &mut Criterion) {
    ...
    group.bench_function("bloom_filter_decrypt_2048", |b| {
        b.iter(|| {
            encrypted_bf.decrypt(black_box(&sk), black_box(&pk))
        });
    });
...
fn server_operations_benchmark(c: &mut Criterion) {
    ...
    for doc_count in document_counts.iter() {
        ...
        group.bench_with_input(
            ...
            |b, (server, query_bf)| {
                b.iter(|| {
                    black_box(server.evaluate_query(black_box(query_bf)))
                });
            },
        );
    }
...
fn generate_hardware_comparison_report() -> std::io::Result<()> {
    ...
    let template = HardwareComparisonData {
        software_results: vec![
            OS2BenchmarkResult::new(
...
fn setup_document_scenario(doc_count: usize, keywords_per_doc: usize) -> (Os2Client, CloudServer, Vec<EncryptedDocument>) {
    let client = Os2Client::new();
    let mut server = CloudServer::new();
    server.receive_paillier_pk(client.get_paillier_pk_for_server());
    ...
    for i in 0..doc_count {
        ...
        for j in 0..keywords_per_doc {
            keywords.push(format!("keyword_{}", (i + j) % 20)); // Cycle through 20 different keywords
        }
        
        // Convert to &str for the function call
        let keyword_refs: Vec<&str> = keywords.iter().map(|s| s.as_str()).collect();
        
        let encrypted_doc = client.outsource_document(&doc_id, &content, keyword_refs).unwrap();
...
fn hardware_comparison_benchmark(c: &mut Criterion) {
    ...
    group.bench_function("single_aes_gcm_encrypt_1kb", |b| {
        b.iter(|| {
            key.encrypt(black_box(&plaintext)).unwrap()
        });
    });
    ...
    group.bench_function("single_aes_gcm_decrypt_1kb", |b| {
        b.iter(|| {
            key.decrypt(black_box(&ciphertext), black_box(&nonce)).unwrap()
        });
    });
